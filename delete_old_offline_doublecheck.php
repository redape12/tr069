...
function checkCpeExists($cpeId) {
    ...
    $response = callDmsMethod('GetCPEs', $payload, 30);
    
    if (!$response['success']) {
        return ['exists' => false, 'error' => 'API error'];
    }
    
    // Проверяем, есть ли устройство в ответе
    if (isset($response['data']['result']['cpe_list']) && !empty($response['data']['result']['cpe_list'])) {
        return ['exists' => true, 'data' => $response['data']['result']['cpe_list'][0]];
    }
